...
public enum Overlay {;
    ...
    public static void overlayPDF(final PDDocument template, final LayoutPage overlay) throws IOException {
        for (final PDPage page : template.getPages()) {
            final COSDictionary pageDictionary = page.getCOSObject();
            final COSBase contents = pageDictionary.getDictionaryObject(CONTENTS);

            final OverlayCOSArray contentArray = new OverlayCOSArray();
    ...
    private static COSName createOverlayXObject(final PDPage page, final LayoutPage layoutPage, final COSStream contentStream) {
        final PDFormXObject xobjForm = new PDFormXObject(contentStream);
    ...
    private static COSStream createStream(final String content) throws IOException {
        ...
        try (final OutputStream out = stream.createOutputStream(FLATE_DECODE)) {
